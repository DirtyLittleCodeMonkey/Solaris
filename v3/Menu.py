...
class Menu:
    def __init__(self, fonts, buttons, title, title_pos, context=None, context_pos=None):
        self.fonts = fonts
...
def pause_menu(fonts, resolution, action_listener):
    centerx = resolution[0] / 2
    centery = resolution[1] / 2
    return_button = MenuButton(fonts, 'Return to game', [centerx, centery - 40], action_listener, 'play')
    ...
    buttons = [return_button, mmb, new_sys]
